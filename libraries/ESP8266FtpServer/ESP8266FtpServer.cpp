...
void FtpServer::clientConnected()
{
  ...
  #endif
  client.println( "220--- Welcome to FTP AMS for ESP8266 ---");
...
{
  #ifdef FTP_DEBUG
	Serial.println("FTP disconnecting client");
  #endif
  abortTransfer();
...
boolean FtpServer::userPassword()
{
  ...
  {
    #ifdef FTP_DEBUG
      Serial.println( "FTP OK. Waiting for commands");
    #endif
...
boolean FtpServer::processCommand()
{
  ...
  else if( ! strcmp( command, "PASV" ))
  {
    ...
    #ifdef FTP_DEBUG
	    Serial.println("FTP connection management set to passive");
      Serial.println("FTP data port set to " + String(dataPort));
    #endif
  ...
  else if( ! strcmp( command, "RETR" ))
  {
    ...
    else if( makePath( path ))
	{
      ...
      {
        #ifdef FTP_DEBUG
		      Serial.println("FTP sending " + String(parameters));
        #endif
  ...
  else if( ! strcmp( command, "STOR" ))
  {
    ...
    else if( makePath( path ))
    {
      ...
      {
        #ifdef FTP_DEBUG
          Serial.println( "FTP receiving " + String(parameters));
        #endif
        client.println( "150 Connected to port " + String(dataPort));
        millisBeginTrans = millis();
        bytesTransfered = 0;
  ...
  else if( ! strcmp( command, "RNFR" ))
  {
    ...
    else if( makePath( buf ))
    {
      ...
      {
        #ifdef FTP_DEBUG
		      Serial.println("FTP renaming " + String(buf));
        #endif
  ...
  {  
    ...
    else if( makePath( path ))
    {
      ...
      {          
            #ifdef FTP_DEBUG
		          Serial.println("FTP renaming " + String(buf) + " to " + String(path));
            #endif
...
boolean FtpServer::dataConnect()
{
  ...
  if (!data.connected())
  {
	  ...
	  if (dataServer.hasClient()) {
		  ...
		  data = dataServer.available();
			#ifdef FTP_DEBUG
		      Serial.println("FTP dataserver client...");
			#endif
...
void FtpServer::abortTransfer()
{
  if( transferStatus > 0 )
  {
    ...
    #ifdef FTP_DEBUG
      Serial.println( "FTP transfer aborted!") ;
    #endif
  }
  transferStatus = 0;
}
